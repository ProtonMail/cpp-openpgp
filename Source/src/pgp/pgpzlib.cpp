...
int zlib_compress(const std::string & src, std::string & dst, int windowBits, int level)
{
    ...
    unsigned int len = (unsigned int )src.size(); //Notes: Convert
    ...
    do {
        ...
        strm.avail_in = ((index + ZLIB_CHUNK) < len)?ZLIB_CHUNK: (uInt)(len - index); //Notes: Convert
...
int zlib_decompress(const std::string & src, std::string & dst, int windowBits)
{
    ...
    do {
        ...
        strm.avail_in = ((index + ZLIB_CHUNK) < len)?ZLIB_CHUNK:(uInt)(len - index);
        for(unsigned int i = 0; i < strm.avail_in; i++){
            in[i] = src[i + index];
        }
        index += strm.avail_in;
