...
std::string Tag1::show(const uint8_t indents, const uint8_t indent_size) const{
    ...
    if (pka < 4){
        out << std::string(tab, ' ') << "    RSA m**e mod n (" /* << bitsize(mpi[0])*/ << " bits): " << mpi[0];
    }
    else if (pka == 16){
        out << std::string(tab, ' ') << "    Elgamal g**k mod p (" /*<< bitsize(mpi[0])*/ << " bits): " << mpi[0] << "\n"
            << std::string(tab, ' ') << "    Elgamal m * y**k mod p (" /*<< bitsize(mpi[1]) */<< " bits): " << mpi[1];
    }
    return out.str();
}
...
std::vector <std::string> Tag1::get_mpi() const{
    return mpi;
}
...
void Tag1::set_mpi(const std::vector <std::string> & m){
    mpi = m;
    size = raw().size();
}
