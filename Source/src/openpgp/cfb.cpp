...
std::string OpenPGP_CFB_decrypt(SymAlg::Ptr & crypt, const uint8_t packet, const std::string & data,  bool isReturnPrefix){
    const unsigned int BS = crypt -> blocksize() >> 3;
...
{
    return ProtonMail::openssl_aes_cfb_256_encrypt(data, key);
}

std::string use_OpenPGP_CFB_decrypt(const std::string & data, const std::string & key)
{
    return ProtonMail::openssl_aes_cfb_256_decrypt(data, key);
}
...
std::string use_OpenPGP_CFB_decrypt(const uint8_t sym_alg, const uint8_t packet, const std::string & data, const std::string & key, bool isReturnPrefix){
    ...
    SymAlg::Ptr alg = use_sym_alg(sym_alg, key);
    return OpenPGP_CFB_decrypt(alg, packet, data, isReturnPrefix);
}
