...
std::string Twofish::run(const std::string & data, bool enc){
    ...
    blk[0] = (unsigned int)toint(little_end(data.substr(0, 4), 256), 256); //Notes: Convert
    blk[1] = (unsigned int)toint(little_end(data.substr(4, 4), 256), 256);
    blk[2] = (unsigned int)toint(little_end(data.substr(8, 4), 256), 256);
    blk[3] = (unsigned int)toint(little_end(data.substr(12, 4), 256), 256); //Notes: Convert
...
void Twofish::setkey(const std::string & KEY){
    ...
    for( uint8_t i = 0; i < (k_len<<1); i++ ){
       in_key[i] = (unsigned int)toint(little_end(KEY.substr(i*4, 4), 256), 256); //Notes: Convert
    }
